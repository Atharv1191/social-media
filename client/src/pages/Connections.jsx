          ...
          {dataArray.map((tab) => {
            ...
            return (
              <button
                ...
                key={tab.label}
                className={`cursor-pointer flex items-center gap-2 px-4 py-2 rounded-md text-sm transition-all ${
                  isActive
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4'>
...

                  {
                    currentTab === "Following" && (
                      <button onClick={()=>handleUnfollow(user._id)} className='w-full p-2 text-sm rounded bg-slate-100 hover:slate-200 text-black active:scale-95 transition cursor-pointer'>
                        Unfollow
                      </button>
                    )
                  }
                   {
                    currentTab === "Pending" && (
                      <button onClick={()=>acceptConnection(user._id)} className='w-full p-2 text-sm rounded bg-slate-100 hover:slate-200 text-black active:scale-95 transition cursor-pointer'>
                        Accept
                      </button>
                    )
                  }
                   {
                    currentTab === "Connections" && (
                      <button onClick={()=>navigate(`/messages/${user._id}`)} className='w-full p-2 text-sm rounded-lg bg-slate-100 hover:slate-200 text--slate-800 active:scale-95 transition cursor-pointer flex items-center justify-center gap-1'>
